...
final class WindowManager: ObservableObject {
    ...
    func refreshWindows() {
        ...
        // Remove IDs that no longer exist from the ordered list (window was closed)
        let stillValid = Set(all.map(\.id))
        orderedTrackedIDs = orderedTrackedIDs.filter { stillValid.contains($0) || !trackedSet.contains($0) }
    }
